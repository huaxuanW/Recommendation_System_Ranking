...
class DeepFM(nn.Module):
    def __init__(self, cate_fea_nuniqs, nume_fea_size=0, emb_size=8, 
        ...
        # for lstm
        self.lstm = LSTM(output_dim=1)

        # for output 
        self.dnn_linear = nn.Linear(hid_dims[-1], num_classes)
        self.sigmoid = nn.Sigmoid()
        
    def forward(self, X_sparse, X_dense=None, X_hist=None):
        """
        ...
        """LSTM部分"""
        lstm_out = self.lstm(X_hist.view(-1, 90, 1))

        out = fm_1st_part + fm_2nd_part + dnn_out + lstm_out   # [bs, 1]
        out = self.sigmoid(out)
        return out

class LSTM(nn.Module):
    def __init__(self, input_size=1, lstm_units=128, hidden_dim2= 256,output_dim=2, n_layers=2, bidirectional=True):
        super().__init__()
        ...
        self.fc1 = nn.Linear(lstm_units * 2, hidden_dim2)
